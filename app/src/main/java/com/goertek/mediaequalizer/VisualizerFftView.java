...
public class VisualizerFftView extends View {
    ...
    public VisualizerFftView(Context context) {
        ...
        mLineWidth = 10;
        mBaseHeight =  50;
        mLineIsSingleColor = false;
        mFirstPartColor =  Color.parseColor("#79ffbe");
        mSecondPartColor =  Color.parseColor("#79ffbe");
        mThirdPartColor =  Color.parseColor("#183326");
        mFourthPartColor = Color.parseColor("#183326");
        init();
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        final int baseX;
        if (mSpeceNum != 0) {
            baseX = mRect.width() / mSpeceNum;
        } else {
            baseX = 40;
        }
        final int baseLine = mRect.height();

        for (int i = 0; i < mSpeceNum; i++) {            //依次用画线的方式绘制每个小矩形，矩形宽度就是线宽
            if (mBytes[i] < 0) {
                mBytes[i] = 127;
            }
